...
public class TEST_V3 {
	...
	public static ArrayList<String> getMesci(Map<String, Integer> mymap) {
	...
	public static void main(String[] args) {
		...
		*/
		...
		for (String item : getMesci()) {
			// this.mesec = item.toLowerCase();
			System.out.printf("Mesec: s%", item);
			// System.out.printf("Mesec: %s, dni na mesec: %d", item, this.getStDni());
			crt(80);
		*/	
		
		ArrayList<String> mymesci = getMesci(dni_v_mescu);
		
		for (int i=0; i < mymesci.size(); i++) {
			System.out.println(mymesci.get(i));
		}
